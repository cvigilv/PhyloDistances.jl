#!/usr/bin/env Rscript
#
# Times TreeDist's JaccardRobinsonFoulds (the k=1, allowConflict=TRUE default, which is
# also what NyeSimilarity computes) on the trees written by run.jl, and appends one row
# per case to results_jrf_r.tsv. Both sides read the same Newick files and both exclude
...
for (n in c(10L, 50L, 200L, 1000L)) {
  ...
  secs <- time_median(function() JaccardRobinsonFoulds(t1, t2))
  mb <- gc_megabytes(function() JaccardRobinsonFoulds(t1, t2))
