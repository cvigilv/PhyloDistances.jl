#!/usr/bin/env Rscript
#
# Times TreeDist's Robinson-Foulds on the trees written by run.jl, and appends one row per
# case to results_r.tsv. Both sides read the same Newick files and both exclude parsing, so
...
for (n in c(10L, 50L, 200L, 1000L)) {
  ...
  secs <- time_median(function() RobinsonFoulds(t1, t2))
  mb <- gc_megabytes(function() RobinsonFoulds(t1, t2))
  rows[[length(rows) + 1L]] <- data.frame(
    case = "pair", n = n, ntrees = 2L, seconds = secs, megabytes = mb
  )
}

allfile <- file.path(treedir, "collection.nwk")
if (file.exists(allfile)) {
  trees <- read.tree(allfile)
  secs <- time_median(function() RobinsonFoulds(trees), budget = 1.0)
  mb <- gc_megabytes(function() RobinsonFoulds(trees))
  rows[[length(rows) + 1L]] <- data.frame(
    case = "allpairs", n = length(trees[[1]]$tip.label), ntrees = length(trees),
    seconds = secs, megabytes = mb
  )
}
