...
"""Time mutual clustering information and its complementary distance."""
function benchmarkclusteringinfo()
    rows = NamedTuple[]
    ...
    for n in RF_SIZES
        ...
        for (metric, comparison) in (
            ...
            trial = @benchmark $comparison($t1, $t2) samples = 200 seconds = 3
            push!(rows, (
                metric = metric, case = "pair", n = n, ntrees = 2,
                seconds = minimum(trial).time / 1e9,
                megabytes = minimum(trial).memory / 1024^2,
                allocs = minimum(trial).allocs,
                value = comparison(t1, t2),
            ))
        end
    end
...
function readtsv(path)
    ...
    return map(lines[2:end]) do line
        ...
        (
            metric = haskey(vals, "metric") ? strip(vals["metric"], '"') : "",
...
function clusteringsection(io, title, metric, rows, rrows)
    println(io, "\n## $title, one pair of trees\n")
...
function report(
    rf, quartet, jrf, inforf, clusteringinfo,
    rrf, rquartet, rjrf, rinforf, rclusteringinfo, rinfo,
)
    io = IOBuffer()
...
function main()
    ...
    @info "Benchmarking clustering information"
    clusteringinfo = benchmarkclusteringinfo()
    ...
    clusteringfile = joinpath(HERE, "results_clusteringinfo_r.tsv")
    banners = String[]
    ...
    if Sys.which("Rscript") === nothing
        ...
        @info "Benchmarking TreeDist's clustering information metrics"
        clusteringinfoinfo = runR("clusteringinfo.R", clusteringfile)
        isnothing(clusteringinfoinfo) || push!(banners, clusteringinfoinfo)
    end

    report(
        rf, quartet, jrf, inforf, clusteringinfo,
        readtsv(rffile), readtsv(qfile), readtsv(jrffile), readtsv(inforffile),
        readtsv(clusteringfile), banners,
    )
