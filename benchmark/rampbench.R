#!/usr/bin/env Rscript
#
# Times TreeDist's JaccardRobinsonFoulds and NyeSimilarity, and Quartet's quartet distance,
# on the trees written by rampbench.jl, and writes one row per case to the output TSV. Both
# sides read the same Newick files and both exclude parsing, so only the distance
# computation is compared.
#
# The distance itself is reported alongside the timing so that rampbench.jl can check the
# two implementations agree on every tree pair it benchmarked.
...
quartet_ceiling <- as.integer(args[[4]])
...
time_median <- function(f, budget = 1.0, min_reps = 5L) {
  times <- numeric(0)
  start <- Sys.time()
  repeat {
    t0 <- Sys.time()
...
for (n in sizes) {
  ...
  fns <- list(jrf = function() JaccardRobinsonFoulds(t1, t2),
              nye = function() NyeSimilarity(t1, t2))
  if (n <= quartet_ceiling) fns$quartet <- function() quartet_distance(t1, t2)

  for (metric in names(fns)) {
    f <- fns[[metric]]
    message(metric, " at ", n, " taxa")
    rows[[length(rows) + 1L]] <- data.frame(
      metric = metric, n = n, seconds = time_median(f),
      megabytes = gc_megabytes(f), value = as.numeric(f())
    )
  }
}
...
cat("TreeDist", as.character(packageVersion("TreeDist")),
    "| Quartet", as.character(packageVersion("Quartet")),
