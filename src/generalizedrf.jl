...
function _compare(::NyeSimilarity, ::Convention, t1, t2)
    ...
    scorer(a, b, n) = _jaccardscore(a, b, n; k = 1, allowconflict = true)
    return splitmatching(scorer, s1, s2; maximize = true).score
end
...
function _compare(m::JaccardRobinsonFoulds, ::Convention, t1, t2)
    index = taxonindex(t1, t2)
    s1, s2 = splits(t1, index), splits(t2, index)
    scorer(a, b, n) = _jaccardscore(a, b, n; k = m.k, allowconflict = m.allowconflict)
    similarity = splitmatching(scorer, s1, s2; maximize = true).score
